...
# 할리우드
# 제약조건
# 1. 규모가 큰 문제를 어떤 추상적인 행태(객체, 모듈 또는 이와 비슷한)를 사용해 개체로
#    분해한다.
# 2. 어떤 행동을 하기 위해 개체를 절대로 직접 호출하지 않는다.
# 3. 개체는 다른 개체가 콜백을 등록할 수 있는 인터페이스를 제공한다.
# 4. 어떤 계산 시점에 개체는 콜백으로 등록해 둔 다른 개체를 호출한다.
    ...
    def __produce_words(self):
        ...
        for w in data_str.split():
            if not self._stop_word_filter.is_stop_word(w):
                for h in self._word_evnet_handlers:
                    h(w)

    def register_for_word_event(self, handler):
        self._word_evnet_handlers.append(handler)


class StopWordFilter:
    """
    Models the stop filter
    """
    _stop_words = []
